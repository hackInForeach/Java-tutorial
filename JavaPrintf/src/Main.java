
public class Main {

	public static void main(String[] args) {
		// printf() = an optional method to control, format and display text to the
		// console window
		// two argument = format string + (object/variable/value)
		// % [flags] [precision] [width] [conversion-character]

		System.out.printf("This is a format string: %d, %.2f, %s, %c, %b", 123, 2.4, "add", 'A', true);
		System.out.println("\n");

		boolean myBoolean = true;
		char myChar = '@';
		String myString = "Dogukan";
		int myInt = 50;
		double myDouble = 1000.234512;

		System.out.printf("%b, %c, %s, %d, %f", myBoolean, myChar, myString, myInt, myDouble);
			
		System.out.println("\n");
		// []width	
		//	minimum number of characters to be written as output
			System.out.printf("Hello %15s",myString);
			
		System.out.println("\n");
		//[precision]		
		// sets number of digits of precision when outputting floating-point values	
		 System.out.printf("You have this much money %.2f",myDouble);
		 System.out.println("\n");
		 System.out.printf("You have this much money %20f",myDouble);
		 System.out.println("\n");
		 System.out.printf("You have this much money %020f",myDouble);
		 System.out.println("\n");
		 System.out.printf("You have this much money %+f",myDouble);
		 System.out.println("\n");
		 System.out.printf("You have this much money %,f",myDouble);
		System.out.println("\n");
		
		//[flags]
		//  adds an effect to output based on the flag added to format specifier
		//	- : left - justify	
		//  + : output a plus ( + ) or minus ( - ) sign for a numeric value
		//  0 : numeric values are zero-padded
		//  , : comma grouping separator if numbers > 1000
		
		
	}

}

















